...
async def run_test():
    ...
    try:
        ...
        # -> Clear email and password fields, input pro@test.com and correct password, then submit login form.
        frame = context.pages[-1]
        # Clear email field
        elem = frame.locator('xpath=html/body/div/div/div/div/form/div/input').nth(0)
        ...
        # -> Clear email and password fields, input promax@test.com and password 'password123', then submit login form.
        frame = context.pages[-1]
        # Clear email field
        elem = frame.locator('xpath=html/body/div/div/div/div/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('')
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Agent Configuration Saved Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The AI receptionist agent setup did not complete successfully. The agent configuration was not saved with all data persisted as required by the test plan.")
